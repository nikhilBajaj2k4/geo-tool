...
if(ev.type==="start"){st(ev.total);HE.mock=!!ev.mock;HE.engs=ev.engs||1;$("rnp").textContent="0/"+ev.total;if(ev.mock)$("rnc2").textContent="mock"}else if(ev.type==="query"){RC=ev.run_cost||0;var pct=gt()?Math.round((ev.done/gt())*100):0;$("rnf").style.width=pct+"%";$("rnp").textContent=ev.done+"/"+gt();$("rntm").textContent=EL();if(ev.mentions&&ev.mentions.length)$("rnl").innerHTML+='<div class="hl">\u2713 '+E(ev.query)+" \u2192 "+E(ev.mentions.join(", "))+(ev.engine?" <span style=color:var(--text3)>["+ev.engine+"]</span>":"")+"</div>";else $("rnl").innerHTML+='<div class="ml">\u2717 '+E(ev.query)+" (none)"+(ev.engine?" <span style=color:var(--text3)>["+ev.engine+"]</span>":"")+"</div>";if(!HE.mock)$("rnc2").textContent=F(RC);$("rnl").scrollTop=$("rnl").scrollHeight}else if(ev.type==="error"){$("rnl").innerHTML+='<div class="el">ERROR: '+E(ev.query)+"</div>"}else if(ev.type==="done"){SRS(false);$("status").textContent="";$("running").style.display="none";$("result").style.display="block";AD=ev.result;RF(ev.result);AH(ev.result);$("result").scrollIntoView({behavior:"smooth"})}}}
...
// render
window.RF=function(d){if(!d)return;AEID="all";try{AD=d;
